...
namespace Gpm.CacheStorage
{
    public class StringToValueHttpTime : StringToValue<DateTime>
    {
        ...
        {
            DateTime dateTime;
            if (DateTime.TryParseExact(text, "ddd, dd MMM yyyy HH:mm:ss Z", CultureInfo.InvariantCulture, DateTimeStyles.AdjustToUniversal, out dateTime) == true)
            {
                return dateTime;
            }

            if (DateTime.TryParse(text, CultureInfo.InvariantCulture, DateTimeStyles.AdjustToUniversal, out dateTime) == true)
            {
                return dateTime;
            }
